...
def softmax(logits):
	"""
	Applies the softmax transformation row-wise to array of logits

	Args:
		logits: array of logits with shape (n_examples, n_classes)
	
	Returns:
		Array of class probabilities with shape (n_examples, n_classes).
		Use np.argmax(softmax(logits), axis=1) to get the predicted class.
	"""

	assert logits.shape[1] > 1, "Do not use softmax with binary labels"
...
def logits_samples2pred_labels(logit_samples):
	"""
	Get labels from MC samples of logits by doing softmax followed by argmax.
	DON'T USE FOR BINARY CASE - THIS USES SOFTMAX

	Args:
		logit_samples: array of logit samples with shape (n_mc_samples, n_examples, n_classes).
						(This is the shape returned by the BNN method sample_logits).
	
	Returns:
		An array of predicted labels with shape (n_mc_samples, n_examples).
	"""
	return np.argmax(np.array([softmax(logit_samples[i,:,:]) for i in range(logit_samples.shape[0])]), axis=2)

def sampled_accuracies(logit_samples, labels):
	"""
	Get the sampled accuracies over the entire test set from logit samples. 

	Args:
		logit_samples: array of logit samples with shape (n_mc_samples, n_examples, n_classes).
						(This is the shape returned by the BNN method sample_logits). n_classes should be 
	...
	assert logit_samples.shape[1]==labels.shape[0]
	assert logit_samples.shape[2]==labels.shape[1]

	if not binary_labels:
		sampled_test_accuracies = np.sum(
			logits_samples2pred_labels(logit_samples) == np.argmax(labels, axis=1), axis=1)/float(labels.shape[0])
		
	else:
		proba_samples = 1.0/(1.0+np.exp(-logit_samples))
		sampled_test_accuracies = np.sum((proba_samples[:,:,0]>0.5) == labels[:,0], axis=1)/float(labels.shape[0])

	return sampled_test_accuracies

def accuracy_hist(logits, labels):
	"""
	Plot a histogram showing test accuracies.
	Just calls sampled_accuracies then plots the result.
	"""
	sampled_acc = sampled_accuracies(logits, labels)
	avg_accuracy = round(np.mean(sampled_acc) * 100, 3)
	print "average accuracy across " + str(logits.shape[0]) + " samples: " + str(avg_accuracy) + "%\n"
